...
- The region is the whole United Kingdom rather than a free-draw area around the buildings, which settles the earlier question of one region or two: one serves both Grove Cottage and Holywell Barn. Recorded in `OUTSTANDING.md` section 7 as a deliberate choice, with what a split would involve.

### 2026-09-07 19:35:03 CST
- **ARC UK feeds added to the staleness monitor and status page** - `check_staleness.py` now watches `omnisense_uk`, `openmeteo_grove` and `openmeteo_holywell` beside the Tanzanian sources, with a per-sensor drill-down for the four UK Omnisense loggers exactly as Tanzania has. The two Omnisense feeds and the three Open-Meteo feeds gained a region in their labels so they can be told apart. `status.html` needed no change, since it iterates whatever `data/status.json` contains. This closes the gap that let the UK fetch fail on 7 September without anything raising it.
- **Confirmed the Tanzanian Omnisense gateway has stopped reporting** - The first run with the UK sources in place put the two side by side and made it plain. The fetch succeeds - 510 897 rows - but the newest Tanzanian reading is 4 September and the ARC weather station's last reading is 3 August, thirty-five days earlier, while the UK site is current to within twelve hours. That is a site-side gateway problem, not a pipeline one. Recorded as `OUTSTANDING.md` section 6.
- **UK Copernicus data staged, CMIP6 still needed** - `data/hist_proj_uk/` holds `t-ERA5_timeseries_historic.csv`, which is correct: absolute annual mean for the United Kingdom region, 1940-2025, around 8-10 C. The five CMIP6 files supplied could not be used: they were exported with the variable on "Change (relative to 1850-1900)", so they carry anomalies of about -1.4 to +3.9 K rather than absolute temperatures. At 2100 under SSP2-4.5 the supplied file reads 1.82 where Tanzania's reads 28.24 C, so against an ERA5 line near 9 C they would appear to show the UK cooling. Their identity was confirmed regardless - the five model sets match the Tanzanian scenarios exactly (9, 22, 23, 22, 27 models), so all five scenarios are present and none is duplicated. A README in the folder records what to re-export. Until the projections arrive the UK datasets keep no `climate_region`, so Long-Term Mode stays hidden there rather than showing Tanzania's.

### 2026-09-07 18:32:21 CST
- **Fix: the daily workflow never committed the UK data it fetched** - The commit step staged a hardcoded list of paths: `data/openmeteo/`, `data/status.json`, `index.html`, `status.html`, plus `data/omnisense/` and `data/cycles/` if present. Adding `data/omnisense_uk/`, `data/openmeteo_grove/` and `data/openmeteo_holywell/` did not update that list, so every UK fetch was written in the runner and then discarded. `data/sensor_snapshot.json` and `data/loggers.json` were missed too, though the `--auto` build rewrites both. It now stages `-A data/`, which is safe because `.gitignore` ignores `data/*` bar an explicit allowlist, and picks up future feeds automatically. Confirmed by dry run that this stages nothing unwanted: `.DS_Store`, the raw `.xlsx` folders and every `legacy/` directory stay ignored.
- **Fix: the push retry could not recover from a dirty tree** - The retry does `git pull --rebase`, which refuses to run with unstaged changes, so on 7 September the run failed with "cannot pull with rebase: You have unstaged changes". Now `--autostash`.
- **PNG export legend names the building in a region view** - Two buildings can hold the same room name: ARC UK has a "Living Room" in each, and ARC Tanzania a "Bedroom 1" in each. The sidebar disambiguates with building sub-headings but an exported legend has no such grouping, so a row now carries `[Grove Cottage]` beside the name, next to the existing grey ID. Single-building datasets are untouched, since the tag would be redundant. Govee, which has no meaningful serial and so skipped the ID, now takes a building tag without one.
- **Checked the PNG title margin across every chart state** - Eleven combinations of dataset, chart type, season lines and Long-Term Mode. The relayout paths (histogram, adaptive comfort) were always safe at 85px and 60px. The injected paths were the exposed ones: line and periodic sat at 10px whenever season labels were absent, giving a title top edge of -2px. All eleven now clear, the tightest being 11px.

### 2026-09-07 17:27:29 CST
- **Fix: Long-Term Mode showed Tanzanian climate data under UK buildings** - Not merely a wrong title. The Copernicus ERA5 and CMIP6 SSP series in `data/hist_proj/` are drawn for one user-defined region in the Climate Atlas, and that region is Tanzanian: its ERA5 series averages about 25 C, which is meaningless over Herefordshire. Selecting ARC UK and enabling Long-Term Mode plotted Dar es Salaam's climate and projections as though they were the UK's. `CLIMATE_REGIONS` now maps a region key to a folder and label, each dataset names the region that applies to it, and `HISTORIC` becomes `HISTORIC_BY_REGION` resolved per dataset through `historicData()`. Datasets with no climate region hide the Long-Term Mode section entirely, and switching into one leaves the mode rather than carrying it. The chart title takes the region's label instead of a hardcoded "Dar es Salaam".
- **Fix: PNG export clipped the chart title whenever season labels were absent** - The title is injected into the exported SVG centred in the chart's top margin. That margin is 85px when season labels are drawn but collapses to 10px when they are not, putting a 14px title's baseline at y=5 and its top edge at -2: clipped by the SVG edge. This hit Long-Term Mode, which turns season lines off, and every UK line chart, whose one-month window contains no season boundary. The line and periodic export now widens the top margin to 36px for the capture and restores it afterwards, matching what the histogram and comfort exports already did. Charts that already have room keep the existing no-relayout fast path.
- **`OUTSTANDING.md` section 5** records what adding UK Copernicus data would involve, including that Grove and Holywell are 150 km apart and so strictly warrant separate regions.

### 2026-09-07 17:06:32 CST
- **Adaptive comfort hover now gives the reading's date and time** - The comfort chart plots room temperature against running mean, so time is not an axis and a point's date was not recoverable from its position at all. The hover carried logger, running mean, room temperature, external source and sensor, but never said *when*. It now leads with `Date / Time`, formatted in site-local time via `toLocalString`. Applied to both the per-logger scatter and the merged compare-mode trace, whose `customdata` each became a two-element array. Verified by driving `Plotly.Fx.hover` in a headless browser and reading the rendered tooltip on Grove, ARC UK, House 5 and a compare set.
- **Logger rows no longer wrap into a ragged second column** - `.cb-label` is a flex row, so the logger name and its `(Open-Meteo)` source suffix were separate flex items and wrapped independently: a long name broke over two lines while the suffix stacked beside it. Name and suffix now sit inside one `.cb-text` block and wrap as continuous text, with the checkbox and colour swatch pinned as fixed items. The sidebar also widens from 300px to 330px, since the UK logger names are the longest in the project. Compare mode's rows use the same structure.
...
- **Holywell `0E3C12EC` is in the living room** - Confirmed, and renamed from "Internal Ambient" to "Living Room". This settles the threshold question: TM59's only fixed figure, 26 C, applies to **bedrooms at night**, and both UK sensors are now known to be in living rooms. `"threshold": None` for the UK datasets is therefore a decision rather than a placeholder. Recorded in `OUTSTANDING.md` section 1, along with the TM52 Category II question in section 2.
- **`debug-omnisense.yml` takes a site input** - So the UK fetch can be exercised in CI against the real credentials without waiting for the nightly run or committing anything.
