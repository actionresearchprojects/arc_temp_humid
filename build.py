...
/* Applicability limits of the adaptive comfort method. Always visible while the
...
// The statistics view is a table, not a plotted figure, so it exports the table
// it is showing rather than the (empty) chart element.
...
function exportCurrentXlsx() {
  if (state.chartType === 'beta-stats') { downloadStatsXlsx(); return; }
  exportChartXlsx();
}
...
function setupStaticListeners() {
  ...
  function handleChartTypeChange(newType) {
    ...
    // The statistics view has no plotted figure to render, so PNG drops out of
    // the export menu and CSV exports the table instead.
    const pngOpt = document.querySelector('#download-menu option[value="png"]');
    if (pngOpt) pngOpt.style.display = isStats ? 'none' : '';
...
function downloadChartPng() {
    ...
    function injectLegendIDCodes(doc) {
      ...
      doc.querySelectorAll('.legendtext').forEach((textEl, idx) => {
        ...
        // Open-Meteo and the climate series are model data, not a building's
        // sensor, so they get neither tag. Govee has no meaningful serial, so it
        // skips the ID but still earns a building tag in a region view.
        if (isOpenMeteo(lid) || lid.startsWith('climate-')) return;
        const showId = lid !== 'govee';
        const suffix = showId ? ' \u00B7 ' + lid : '';
        const rawText = textEl.textContent;
        const splitAt = suffix ? rawText.indexOf(suffix) : -1;
        const baseName = splitAt >= 0 ? rawText.slice(0, splitAt) : rawText;
        // In a region view the sidebar groups loggers under a building heading,
        // but an exported legend has no such grouping and two buildings can hold
        // the same room name - ARC UK has a "Living Room" in each. Name the
        // building on the legend row so the PNG stands on its own.
        const meta = dataset().meta;
        const bKey = meta.isCombined && meta.loggerBuilding ? meta.loggerBuilding[lid] : null;
        const building = bKey ? t(bKey) : null;
        if (!suffix && !building) return;
        while (textEl.firstChild) textEl.removeChild(textEl.firstChild);
        const t1 = doc.createElementNS(ns, 'tspan');
        t1.textContent = baseName;
        textEl.appendChild(t1);
        if (building) {
          const tb = doc.createElementNS(ns, 'tspan');
          tb.setAttribute('fill', '#777777');
          tb.setAttribute('font-size', '0.85em');
          tb.textContent = ' [' + building + ']';
          textEl.appendChild(tb);
        }
        if (suffix) {
...
// The tables currently on screen as rows. One block per metric, separated by a
// blank row, with the period and sort order recorded up top so the file still
// makes sense once it is away from the dashboard. CSV and XLSX both build from
...
function statsExportRows() {
  ...
  for (const tbl of _statsTables) {
    rows.push([]);
    rows.push([tbl.caption]);
    rows.push(tbl.cols.map(c => c.unit ? `${c.label} (${c.unit})` : c.label));
    for (const r of tbl.rows) {
...
function statsCsvText() { return rowsToCsv(statsExportRows()); }

// Filename describing the dataset, metrics, sensor selection and period, shared
// by both statistics export formats.
function statsExportFilename(ext) {
  ...
  const _n = new Date(), _p = n => String(n).padStart(2, '0');
  const ts = `${_n.getFullYear()}${_p(_n.getMonth() + 1)}${_p(_n.getDate())}_${_p(_n.getHours())}${_p(_n.getMinutes())}`;
  return `ARC_${slug(dsLabel())}_SummaryStats${metricStr}${sensorStr}_${exportRangeStr()}_${ts}.${ext}`;
}

function downloadStatsCsv() {
  if (!_statsTables.length) return;
  triggerCsvDownload(statsCsvText(), statsExportFilename('csv'));
}

function downloadStatsXlsx() {
  if (!_statsTables.length) return;
  triggerXlsxDownload(statsExportRows(), t('betaStatsTitle'), statsExportFilename('xlsx'));
}
...
(function() {
  ...
  const textKeys = {
    ...
    comfort: () => t('infoComfort') + applicabilityNoteHtml(),
  ...
  icon.addEventListener('mouseenter', () => {
    ...
    // innerHTML, not textContent: the comfort entry appends the formatted
    // applicability note. None of these strings contain stray angle brackets.
    tip.innerHTML = fn ? fn() : '';
    const r = icon.getBoundingClientRect();
    tip.style.display = 'block';
    let left = r.left;
...
// The adaptive comfort method's applicability limits, formatted for the dark
...
(function() {
  ...
  items.forEach(({iconId, tipId, key, hasLink, applicability}) => {
    ...
    function showTip() {
      let txt = t(key);
      if (applicability) txt += applicabilityNoteHtml();
